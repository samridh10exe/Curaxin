                 ...
                 def check(EA):
                     if (re.fullmatch(SEA,EA)):
                         ...
                         if(len(password)<8)and\
                             ...
                             AM=input("Enter Your Address:")
                             BL=input("Enter Balance Amount:")

                             query=("Insert into personal_profile values('{}','{}',{},'{}','{}','{}','{}')".format(N,PW,PN,EA,AM,CYP,BL))
                             cur.execute(query)
                             con.commit()
                ...
                if SS != str(ADPASS):
                    ...
                    while k<=0:
                        ...
                        if Y==1:
                            ...
                            MD=input("ENTER THE MANUFACTURER OF THE DRUGS:")
                            PR=input("Enter Medicine Price:")

                            AM="insert into inventory values({},'{}','{}','{}','{}','{}')".format(MID,TM,DM,MN,MD,PR)
                            cur.execute(AM)
                            con.commit()
                            print("MEDICINE ADDED SUCCESSFULLY")
                            k+=0
                        elif Y==2:
                            MNR=input("ENTER MEDICINE NAME WHICH YOU WANT TO REMOVE:")
                            DMN="delete from inventory where MED_NAME ={}".format(MNR)
                            cur.execute(DMN)
                            print("MEDICINE REMOVED SUCCESSFULLY")
                        elif Y==3:
                            VU="select * from personal_profile"
                            cur.execute(VU)
                            OP=cur.fetchmany()
                            print(OP)
                        elif Y==4:
                            UN=input("ENTER USERNAME:-")
                            RU="delete from personal_profile where USERNAME={}".format(UN)
                            cur.execute(RU)
...
while f<=0:
    ...
    if x==1:
        ...
        R=input(" | Ayurveda | Capsules | Drops | Drugs |\
            ...
            if PRO==1:
                ...
                if UpdatePROF==1:
                    ...
                    NP=input("CHOOSE YOUR PLAN(ENTER Rs AND THE PRICE ONLY)\
                                      ...
                                      \nJOIN CURAXIN FOR 6 MONTH AT Rs1550\-\
                                      \nJOIN CURAXIN FOR ANNUAL MEMBERSHIP AT Rs2749\-\n")
                    PLAN="update PERSONAL_PROFILE Membership_Plan='{}'where Membership_Plan='{}'".format(NP,CP)
                    cur.execute(PLAN)
                    con.commit()
                    print("PLAN UPDATED")
                else:
        ...
        search=input("Search a MEDICINE NAME:")
        FE=(search+"%")
